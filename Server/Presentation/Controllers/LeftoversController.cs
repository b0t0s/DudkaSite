...
[ApiController]
[Route("[Controller]")]
public class LeftoversController : GenericController<Leftovers>
{
    public LeftoversController(
        ILogger<LeftoversController> logger,
    ...
    {
        var endpoint = new PosterEndpoint($"{_options.Value.Domain}", "storage.getStorageLeftovers", $"{_options.Value.Token}").ToString();
        ...
        try
        {
            ...
            var leftovers = DeserializeResponse(responseBody["response"]);

            if (response.IsSuccessStatusCode)
            {
                if (leftovers.IsNullOrEmpty()) return NoContent();
